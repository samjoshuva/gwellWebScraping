...
async function createTitleCopy(env, title, copy) {
  const image = await env.createEntry("titleCopy", {
    fields: {
      title: {
        "en-US": title
      },
...
async function createAuthor(env, name, altText, imgLink) {
  let block = await createImage(env, altText, imgLink);
...
async function createContentBlocksVideos(
  env,
  altText = null,
  videoUrl = null,
  videoId = null
